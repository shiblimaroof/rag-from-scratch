...
write 1-2 sentence that situate this chunk within the overall document.
Be specific - mention names, events, time periods if relavent.
...
print("Adding context to each chunk...\n")
enriched_chunks = []
for i, chunk in enumerate(chunks):
    enriched = add_context(text,chunk)
    enriched_chunks.append(enriched)
    print(f"chunk{i+1}enriched:")
    print(enriched[:300])
    print("-" * 40)

# Embed enriched chunks
model = SentenceTransformer("all-MiniLM-L6-v2")
embeddings = model.encode(enriched_chunks, show_progress_bar= True)
...
# Same query as before — compare results
query = "Why did Paul Graham sell Viaweb?"
query_embedding = model.encode([query]).astype('float32')
distances, indices = index.search(query_embedding, k=3)

print(f"\nQuery: {query}")
print("Top 3 retrieved chunks after contextual enrichment:\n")
for rank, idx in enumerate(indices[0]):
    print(f"Rank {rank+1}:\n{enriched_chunks[idx][:400]}\n{'-'*40}")
...
#     "GPT-2 vocabulary size is 32000",   # wrong number
#     "GPT-2 vocabulary size is 100000",  # very wrong number
#     "the number of tokens in GPT-2",    # no number at all
# ])

# # cosine similarities with the first sentence
# from sklearn.metrics.pairwise import cosine_similarity
# sims = cosine_similarity([vecs[0]], vecs)[0]
